...
def calcular_apuesta_individual():
    ...
    with col2:
        usar_prob_implicita = st.checkbox("Usar probabilidad implícita", value=True)
        if usar_prob_implicita:
            probabilidad_estimada = 1 / cuota
            st.write(f"Probabilidad implícita: {probabilidad_estimada*100:.2f}%")
        else:
    ...
    if st.button("Calcular"):
        try:
            ...
            # Cálculo de Kelly corregido
            if cuota > 1:
                kelly = (probabilidad_estimada * (cuota - 1) - (1 - probabilidad_estimada)) / (cuota - 1)
                kelly = max(0, kelly)
...
def calcular_parlay():
    ...
    for i in range(num_apuestas):
        ...
        with col1:
            ...
            cuotas.append(cuota)
        with col2:
            usar_prob_implicita = st.checkbox(f"Usar prob. implícita {i+1}", value=True, key=f"usar_prob_{i}")
            if usar_prob_implicita:
                prob = 1 / cuota
                st.write(f"Prob. implícita: {prob*100:.2f}%")
            else:
    ...
    if st.button("Calcular Parlay"):
        try:
            ...
            # Cálculo de Kelly para parlay
            if cuota_parlay > 1:
                kelly_parlay = (prob_estimada_parlay * (cuota_parlay - 1) - (1 - prob_estimada_parlay)) / (cuota_parlay - 1)
                kelly_parlay = max(0, kelly_parlay)
